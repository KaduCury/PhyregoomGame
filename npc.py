...
class NPC(AnimatedSprite):
    ...
    def check_hit_in_npc(self):
        if self.phyre_g_value and self.game.player.shot:
    ...
    def run_logic(self):
        if self.alive:
            self.phyre_g_value = self.phyre_g_player_npc()
            self.check_hit_in_npc()

            if self.pain:
                self.animate_pain()

            elif self.phyre_g_value:
    ...
    def draw_phyre_g(self):
        pg.draw.circle(self.game.screen, 'red', (100 * self.x, 100 * self.y), 15)
        if self.phyre_g_player_npc():
